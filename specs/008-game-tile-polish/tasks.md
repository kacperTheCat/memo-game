...
- [x] T002 [P] Verify or implement `src/game/rarityTier.ts` with ordered ladder + `isTopTier` + Vitest `src/game/rarityTier.spec.ts` (**FR-003**, **FR-004**)
- [x] T003 [P] Verify or implement `src/game/animationEasing.ts` (`clamp01`, `lerp`, ease curves) + Vitest `src/game/animationEasing.spec.ts` for rAF-driven motion
- [x] T004 Ensure `SessionSnapshot` / `buildSnapshot` in `src/stores/gameSession.ts` never persists ephemeral animation or strip state; adjust `src/game/memoryTypes.ts` + `src/stores/gameSession.spec.ts` per [data-model.md](./data-model.md)
- [x] T005 Add **board band vs collection strip band** layout to `src/game/canvasLayout.ts` (constants + `boardAreaCssRect(fullW, fullH)` or equivalent) + extend Vitest `src/game/canvasLayout.spec.ts`
- [x] T006 Update `src/game/canvasHitTest.ts` and `src/components/GameCanvasShell.vue` so `cellIndexFromPointer` uses **board** dimensions only and ignores clicks in the strip band ([plan.md](./plan.md))
...
- [x] T008 [P] [US1] Vitest `src/game/canvasTileDraw.neutralBack.spec.ts` — concealed path must not apply catalog `color` / rarity stroke (mock `CanvasRenderingContext2D`)
...
- [x] T010 [US1] Update `src/game/canvasTileDraw.ts` concealed branch: uniform neutral back; no item-colored borders (**FR-001**)
- [x] T011 [US1] Adjust `src/components/GameCanvasShell.vue` / `drawTile` args if concealed path must not receive catalog stroke color
...
- [x] T013 [P] [US2] Vitest new or extend `src/game/collectStripLayout.spec.ts` (or `canvasLayout.spec.ts`) — pure helpers for strip slot positions / scale curve along collect `t∈[0,1]` if factored out
- [x] T014 [P] [US2] Vitest `src/game/tileAnimationBudget.spec.ts` (or new `mismatchPhaseTiming.spec.ts`) — shake + flip-back + collect durations stay within **SC-003** / **SC-004** / **FR-011** vs `src/stores/gamePlay.ts` / `src/game/tileMotionConstants.ts`
...
- [x] T016 [US2] Add SR-only hooks in `src/components/GameCanvasShell.vue`: `data-testid="game-collect-strip"` with `data-collect-count`; optional `data-mismatch-phase` on shell (`idle` \| `shake` \| `flip_back`) per contract
- [x] T017 [US2] Implement ephemeral **collect flight** + runtime **`stripChips`** list + paint in `GameCanvasShell.vue` and `src/game/canvasTileDraw.ts` (**FR-009**); serialize overlapping collects if needed ([research.md](./research.md) §10)
...
- [ ] T021 [P] [US3] Vitest `src/game/tileFaceStyle.spec.ts` — top tier → gold palette vs catalog `#hex` gradients using `src/game/rarityTier.ts`
...
- [ ] T023 [US3] Face background gradients in `src/game/canvasTileDraw.ts` using `entry.color` + `isTopTier(entry.rarity)`; remove old border emphasis (**FR-002**–**FR-004**)
- [ ] T024 [US3] Unified inner-rect image + glass overlay in `src/game/canvasTileDraw.ts` (**FR-005**–**FR-006**)
...
- [x] T026 [P] [US4] Vitest `src/game/tileParallaxSmooth.spec.ts` — smoothing converges toward `parallaxOffset` without instability
...
- [x] T028 [US4] Wire `src/game/tileParallaxSmooth.ts` into `src/components/GameCanvasShell.vue` paint path; respect reduced motion (**FR-007**)
...
- [ ] T032 [P] If **FR-012** not met on baseline hardware, reduce glass/gradient cost in `src/game/canvasTileDraw.ts` before adding npm animation dependencies ([research.md](./research.md) §2)
...
# - src/game/collectStripLayout.spec.ts (or canvasLayout.spec.ts)
# - src/game/tileAnimationBudget.spec.ts
# - e2e/tile-visual-polish.spec.ts (scenarios 3–6)

# Then implement hooks + collect + mismatch in:
# - src/components/GameCanvasShell.vue
# - src/game/canvasTileDraw.ts
# - src/stores/gamePlay.ts / src/game/tileMotionConstants.ts (timing alignment)
```
